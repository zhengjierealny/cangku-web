    ...
    export default {
        ...
        data() {
            let checkDuplicate =(rule,value,callback)=>{
                ...
                this.$axios.get(this.$httpUrl+"/goodstype/findByNo?name="+this.form.name).then(res=>res.data).then(res=>{
                    if(res.code!=200){

                        callback()
                    }else{
                        callback(new Error('该分类名已经存在'));
                    }
                })
            };
            return {
        ...
        methods:{
            ...
            loadPost(){
                this.$axios.post(this.$httpUrl+'/goodstype/listPageS',{
