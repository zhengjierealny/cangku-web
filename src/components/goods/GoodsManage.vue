<template>
    <div>
        <div style="margin-bottom: 5px;">
            ...
            <el-button type="primary" style="margin-left: 5px;" @click="applyGoods" v-if="user.roleId==2">出库申请
            </el-button>
        ...
        >
            ...
            <el-table-column prop="state" label="状态" width="180">
                <template slot-scope="scope">
                    <el-tag
                            :type="scope.row.count < 100 ? 'danger' : 'success'"
                            disable-transitions>{{scope.row.count < 100 ? '库存不足' : '库存良好'}}</el-tag>
                </template>
            </el-table-column>
    ...
    <el-button type="primary" @click="doInGoods">确 定</el-button>
        ...
        <el-dialog
            ...
            <el-form ref="form1" :rules="rules1" :model="form1" label-width="80px">
                ...
                <el-form-item label="备注" prop="remark" v-if="this.user.roleId!=2">
                    <el-col :span="20">
                        <el-input type="textarea" v-model="form1.remark"></el-input>
                    </el-col>
                </el-form-item>
    ...
    <el-button type="primary" @click="doApplyGoods">确 定</el-button>
  </span>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        ...
        data() {
            let checkCount = (rule, value, callback) => {
                ...
                //     callback();
                // }
                this.$axios.get(this.$httpUrl+"/goods/findCount?id="+this.form1.id+ "&count=" + this.form1.count
                ).then(res=>res.data).then(res=>{
                    if(res.code==200){

                        callback()
                    }else{
                        callback(new Error('数量大于库存数'));
                        alert('数量大于库存数');
                    }
                })
            };
        ...
        methods:{
            ...
            inGoods(){
                ...
                this.form1.storage=this.currentRow.storage
            ...
            doMod(){
                this.$axios.post(this.$httpUrl+'/goods/updateMod',this.form).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
            ...
            doInGoods(){
                if(this.form1.action=='2') {
                    if(this.form1.address.trim().length > 0) {
                    ...
                    if(this.form1.count.trim().length > 0 ) {
                        this.$axios.post(this.$httpUrl + '/record/save', this.form1).then(res => res.data).then(res => {
                            console.log(res)
                            if (res.code == 200) {
            ...
            doApplyGoods(){
                if(this.form1.address.trim().length > 0) {
                    this.$axios.post(this.$httpUrl + '/applys/saveU', this.form1).then(res => res.data).then(res => {
                        console.log(res)
                        if (res.code == 200) {
            ...
            loadPost(){
                this.$axios.post(this.$httpUrl+'/goods/listPageS',{
